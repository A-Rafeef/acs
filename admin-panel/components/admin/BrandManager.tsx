...
export default function BrandManager({ initialBrands }: BrandManagerProps) {
  ...
  const handleDelete = async (id: string, brName: string) => {
    if (!confirm(`Are you sure you want to delete "${brName}"? This will unlink brands from related products.`)) return
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.success(`Brand "${brName}" deleted`)
      } else {
                  ...
                  <tr key={br.id} className="hover:bg-secondary/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="p-1 text-foreground/45 hover:text-red-500 transition-colors rounded-sm hover:bg-red-500/5 cursor-pointer"
                      >
                        {deletingId === br.id ? (
      ...
      <div className="border border-border/30 p-6 rounded-sm bg-secondary/5 h-fit space-y-4">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-foreground text-background text-xs font-bold uppercase tracking-widest py-3 flex items-center justify-center gap-2 hover:bg-foreground/80 transition-colors rounded-sm cursor-pointer"
          >
            {submitting ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
