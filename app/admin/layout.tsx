...
import { LayoutDashboard, ShoppingBag, FolderOpen, Tags, LogOut, Loader2 } from 'lucide-react'
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleSignOut = async () => {
    try {
      if (isMock) {
        // Delete mock cookie by setting past expiration
        document.cookie = 'mock-admin-session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT;'
        toast.success('Signed out successfully (Mock Session Cleared)')
  ...
  // If loading user state, show clean loader spinner
  ...
  // Don't render sidebar shell on login screen
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Sidebar navigation */}
      <aside className="w-64 border-r border-border/40 bg-secondary/15 flex flex-col flex-shrink-0">
        {/* Brand header */}
